$(function () {
  ...
  $(document).ready(function () {
    ...
    $("#searchButton").on("click", function () {
      try {
        ...
        $.ajax({
          url: servidor + "admin/buscarLibro",
          method: "POST",
          ...
          success: function (response) {
            ...
            if (libros.length > 0) {
              ...
              $("#card-result").show();
              $("#resultSelect").hide();
              $("#result").text("Lo sentimos, su búsqueda no coincidió con ningún documento.");
    ...
    function insertarFavorito(libroId) {
      $.ajax({
        url: servidor + "admin/guardarFavorito",
        method: "POST",
        data: {
          tipo: "nuevo",
          libro_id: libroId,
  ...
  async function cardsLibros() {
    try {
      let peticion = await fetch(servidor + `admin/cardsLibros`);
  ...
  $(function () {
    ...
    async function loadBooks() {
      try {
        let peticion = await fetch(servidor + `admin/librosfavoritos`);
        let response = await peticion.json();
