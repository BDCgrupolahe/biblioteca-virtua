...
async function tablaLibros() {
  try {
    let peticion = await fetch(servidor + `admin/libroview`);
      ...
      columns: [
        ...
        { data: "anio_libro", className: "text-vertical text-center" },
        {
          ...
          render: function (data, item) {
            const condicion = data.estatus_libro;
            let botones = "";
            ...
            if (condicion == 1) {
              botones = `<div class="col-sm-12 col-md-12 col-lg-12 col-xl-12 justify-content align-items-center">
                          ...
                          <button data-nombre="${data.titulo_libro}" data-condicion="${data.estatus_libro}" data-id="${btoa(btoa(data.id_libro))}" data-bs-toggle="tooltip" title="Desactivar libro" type="button" class="btn btn-success btn-delete-libro">
                          <i class="fa-solid"></i>Activado</button>
  
  
                          <a href="${servidor}pdfjs-4.0.379-dist/web/viewer.html?file=${data.archivo_old}" target="_blank" data-bs-toggle="tooltip"
                          title="Ver libro" type="button" class="btn btn-secondary visualizar-cartas"><i class="fa-solid fa fa-file-word-o"></i></a>
                          </div>`;
            } else {
              botones = `<div class="col-sm-12 col-md-12 col-lg-12 col-xl-12 justify-content align-items-center"><button data-id="${btoa(
                btoa(data.id_libro)
              )}"  data-bs-toggle="tooltip" title="Editar libro" type="button" class="btn btn-info btn-edit-libro">
                            <i class="fa-solid fa-edit"></i>Editar</button>
                ...
                btoa(data.id_libro)
              )}" data-bs-toggle="tooltip" title="Activar libro" type="button" class="btn btn-danger btn-delete-libro">
                            <i class="fa-solid"></i>Desactivado</button>
...
$(function () {
  ...
  function eliminarLibro(id) {
    $.ajax({
      type: "POST",
      url: servidor + `admin/eliminarLibro/${id}`,
      dataType: "json",
      contentType: false,
      cache: false,
      processData: false,
  ...
  async function editarLibro(id) {
    let peticion = await fetch(servidor + `admin/buscaradminLibro/${id}`);
    let response = await peticion.json();
  ...
  async function cat_genero(identificador) {
    try {
      $(identificador).empty();
      let peticion = await fetch(servidor + `admin/cat_genero`);
      let response = await peticion.json();
  ...
  async function cat_autores(identificador) {
    try {
      $(identificador).empty();
      let peticion = await fetch(servidor + `admin/cat_autores`);
      let response = await peticion.json();
      console.log(response);
      let option_select = document.createElement("option");
      option_select.value = "";
  ...
  $(".btn-save-generos").on("click", function () {
    let form = $("#" + $(this).data("formulario"));
    let tipo_form = $(this).data("tipo");
    let url = tipo_form == "nuevo" ? "guardarGenero" : "actualizarEstado";
    if (form[0].checkValidity() === false) {
      event.preventDefault();
      event.stopPropagation();
    } else {
  ...
  //   let form = $("#" + $(this).data("formulario"));
  //   let tipo_form = $(this).data("tipo");
  //   let url = tipo_form == "nuevo" ? "guardarAutores" : "actualizarDocumento";
  //   if (form[0].checkValidity() === false) {
  //     event.preventDefault();
  //     event.stopPropagation();
  //   } else {
