...
class AdminModel extends ModelBase
{
    ...
    public static function cardsLibros()
    {
        try {
            $con = new Database;
    ...
    public static function guardarAutores($datos)
    {
        try {
            $con = new Database;
            $con->pdo->beginTransaction();
            $query = $con->pdo->prepare("INSERT INTO cat_autores (nombre_autor) VALUES (:NombreAutor)");
            $query->execute([

                ':NombreAutor' => $datos['autor']
            ]);
            $idtema_resp = $con->pdo->lastInsertId();
            $con->pdo->commit();
            return $idtema_resp;
            ...
            $con->pdo->rollBack();
            echo "Error recopilado model guardarAutores: " . $e->getMessage();
            return false;
        }
    }
    ...
    public static function librosfavoritos()
    {
        try {
            $con = new Database;
            $query = $con->pdo->prepare("SELECT libros.titulo_libro AS titulo_libro, 
            libros.anio_libro AS anio_libro, 
            libros.editorial_libro AS editorial_libro, 
            libros.nombre_archivo AS nombre_archivo, 
            cat_genero.genero AS genero,    
            libros.nombre_imagen AS nombre_imagen, 
            cat_autores.nombre_autor AS nombre_autor, 
            COUNT(agregacion_favoritos.fk_id_libro) AS contador_favoritos, 
            ROW_NUMBER() OVER (ORDER BY COUNT(agregacion_favoritos.fk_id_libro) DESC) AS ranking FROM libros 
            INNER JOIN agregacion_favoritos ON libros.id_libro = agregacion_favoritos.fk_id_libro 
            INNER JOIN cat_autores ON cat_autores.id_autor = libros.fk_id_autor
            INNER JOIN cat_genero ON cat_genero.id_genero = libros.fk_genero_libro 
            WHERE libros.estatus_libro = 1 AND libros.estatus_permiso = 1 
            GROUP BY libros.id_libro ORDER BY contador_favoritos DESC LIMIT 5;");
            $query->execute();
            return $query->fetchAll();
        } catch (PDOException $e) {
    ...
    public static function librosfavoritostop()
    {
        try {
            $con = new Database;
            $query = $con->pdo->prepare("SELECT libros.titulo_libro AS titulo_libro, 
            ...
            cat_genero.genero AS genero,    
            libros.nombre_imagen AS nombre_imagen, 
            agregacion_favoritos.id_agregacion,
            agregacion_favoritos.fk_id_usuario,
            cat_autores.nombre_autor AS nombre_autor, 
            COUNT(agregacion_favoritos.fk_id_libro) AS contador_favoritos, 
            ROW_NUMBER() OVER (ORDER BY COUNT(agregacion_favoritos.fk_id_libro) DESC) AS ranking FROM libros 
            INNER JOIN agregacion_favoritos ON libros.id_libro = agregacion_favoritos.fk_id_libro 
            ...
            INNER JOIN cat_genero ON cat_genero.id_genero = libros.fk_genero_libro 
            WHERE libros.estatus_libro = 1 AND libros.estatus_permiso = 1 
            GROUP BY libros.id_libro ORDER BY contador_favoritos DESC LIMIT 50;");
            $query->execute();
            return $query->fetchAll();
        } catch (PDOException $e) {
            echo "Error recopilado model librosfavoritos: " . $e->getMessage();
    ...
    public static function guardarLibro($datos)
{
    try {
        ...
        $query = $con->pdo->prepare("INSERT INTO libros (titulo_libro,fk_id_usuario,editorial_libro,fk_genero_libro,fk_id_autor,anio_libro,nombre_archivo,nombre_imagen,ruta_archivo,token,estatus_permiso) VALUES (:TituloLibro,:FkIdUsuario,:EditorialLibro,:GeneroLibro,:IdAutor,:AnioLibro,:NombreArchivo,:imagen,:RutaArchivo,:Token,:EstatusPermiso)");
    ...
    public static function librorevision()
    {
        try {
            $con = new Database;
            $query = $con->pdo->prepare("SELECT libros.id_libro, libros.titulo_libro, libros.editorial_libro, libros.fk_genero_libro, libros.fk_id_autor, libros.anio_libro, libros.token,libros.nombre_imagen, libros.estatus_permiso, cat_autores.id_autor, libros.nombre_archivo AS archivo_old, cat_usuarios.nombre_usuario AS nombre_usuario, cat_autores.nombre_autor AS autor_libro, cat_genero.id_genero, cat_genero.genero AS genero_libro FROM libros INNER JOIN cat_autores ON libros.fk_id_autor = cat_autores.id_autor INNER JOIN cat_usuarios ON libros.fk_id_usuario = cat_usuarios.id_usuario INNER JOIN cat_genero ON libros.fk_genero_libro = cat_genero.id_genero WHERE estatus_permiso = 0;
            ");
            $query->execute();
            return $query->fetchAll();
        } catch (PDOException $e) {
    ...
    public static function buscaradminLibro($id_libro)
    {
        try {
            $con = new Database;
            ...
            echo "Error recopilado model buscaradminLibro: " . $e->getMessage();
            return;
        }
    }
    ...
    public static function guardaradminGenero($datos)
    {
        try {
            $con = new Database;
            $con->pdo->beginTransaction();
            ...
            $con->pdo->rollBack();
            echo "Error recopilado model guardaradminGenero: " . $e->getMessage();
            return false;
        }
    }
    ...
    public static function revisionlibro($id)
    {
        try {
            $con = new Database;
            $con->pdo->beginTransaction();
    
            $query = $con->pdo->prepare("SELECT estatus_permiso FROM libros WHERE id_libro = :id;");
            $query->execute([
                ':id' => base64_decode(base64_decode($id))
            ]);
            $condicionActual = $query->fetch(PDO::FETCH_ASSOC)['estatus_permiso'];
    
            $nuevoValorCondicion = ($condicionActual == 1) ? 0 : 1;

            $query = $con->pdo->prepare("UPDATE libros SET estatus_permiso = :condicion WHERE id_libro = :id;");
            $query->execute([
                ':id' => base64_decode(base64_decode($id)),
                ':condicion' => $nuevoValorCondicion
            ]);
            ...
            $con->pdo->rollBack();
            echo "Error recopilado model revisionlibro: " . $e->getMessage();
            return false;
        }
    }
    ...
    public static function eliminarlibrodesaprovado($id)
    {
        try {
            $con = new Database;
            $con->pdo->beginTransaction();
            $query = $con->pdo->prepare("DELETE FROM libros WHERE id_libro = :id;");
            // echo $query->queryString;  // Imprime la consulta SQL para depuración
            $query->execute([
                ':id' => base64_decode(base64_decode($id))
            ]);
            ...
            $con->pdo->rollBack();
            echo "Error recopilado model eliminarlibrodesaprovado: " . $e->getMessage();
            return false;
        }
    }
